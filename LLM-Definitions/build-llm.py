    ...
    if provider == "Anthropic":
        return {
            ...
            "temperature": {
                "default": 1,
                "range": "0 - 1",
                "description": "Sampling temperature (Anthropic range 0-1). Do not combine with top_p — the Anthropic API accepts only one of the two per request.",
            },
            "API_KEY": {
                "default": "Anthropic",
                "range": "sk-ant-****",
...
def make_requirements(provider, hf_repo_id=None, hf_gated=False, folder_name=None):
    ...
    if provider == "Anthropic":
        return [step0, {"step": "install common packages",
                        "command": "pip3 -q install requests>=2.31.0 tiktoken>=0.5.1 anthropic>=0.25.0"}]
    if provider == "Google (Gemini)":
        return [step0, {"step": "install common packages",
                        "command": "pip3 -q install requests>=2.31.0 tiktoken>=0.5.1 google-generativeai>=0.5.0"}]
    if provider == "Meta / HuggingFace (local/SCR)":
        steps = [
            step0,
            {"step": "install huggingface CLI and packages",
             "command": "pip3 -q install huggingface-hub>=0.18.0 transformers torch accelerate numpy==1.26.4"},
        ]
        if hf_gated and hf_repo_id:
            steps.append({
                "step": f"Login with HuggingFace — accept the license at https://huggingface.co/{hf_repo_id} first",
                "command": "hf login --token $(cat /etc/secret-volume/huggingfacetoken)",
            })
        if hf_repo_id:
            local_dir = f"/pybox/model/{folder_name}" if folder_name else "/pybox/model"
            steps.append({
                "step": "download model",
                "command": f"hf download --quiet {hf_repo_id} --local-dir {local_dir}",
            })
        return steps
    return [step0, {"step": "install common packages",
...
if __name__ == "__main__":
    userPrompt   = ["Count to ten in French"]
    systemPrompt = ["You are an AI Assistant helping people learn languages"]
    options = ["{max_tokens:1000,temperature:1,API_KEY:***your_actual_API_key_here***}"]
    response, run_time, prompt_length, output_length = scoreModel(userPrompt, systemPrompt, options)
    print("Response:", response)
    print("Run time:", run_time)
    print("Prompt length:", prompt_length)
...
def main():
    ...
    if provider == "Meta / HuggingFace (local/SCR)":
        while True:
            hf_repo_id = ask("HuggingFace repository ID (e.g. HuggingFaceTB/SmolLM-135M-Instruct)")
            if hf_repo_id:
                break
            print("  Required — cannot be empty.")
        hf_gated = ask("Gated model requiring HuggingFace login? (y/N)", default="n").lower() in ("y", "yes")
        print()
    ...
    _write_json(dest, "requirements.json", make_requirements(
        provider, hf_repo_id=hf_repo_id, hf_gated=hf_gated, folder_name=folder_name))
