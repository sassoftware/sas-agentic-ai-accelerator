...
def scoreModel(userPrompt, systemPrompt, options):
    ...
    messages = [{"role": "system", "content": systemPrompt[0]}, {"role": "user", "content": userPrompt[0]}]
    input_text=tokenizer.apply_chat_template(messages, tokenize=False)
    inputs = tokenizer.encode(input_text, return_tensors="pt").to(device)
    outputs = model.generate(inputs, max_new_tokens=int(options['max_tokens']), temperature=float(options['temperature']), top_p=float(options['top_p']), do_sample=True)
    # Collecting output metrics
    prompt_length = len(inputs[0])
    response = tokenizer.decode(outputs[0][prompt_length + 3:-3])
    # Remove the input prompt from the generated text and the <|im_start|>assistant token
    output_length = len(outputs[0][prompt_length + 3:-3])
