...
Before anything is written, the wizard shows the **catalog-derived values** — option defaults, metadata (description, context length, dates) and token pricing — for you to **confirm or adjust**: these steer scoring behavior and cost monitoring, so they should be accepted consciously. `--accept-defaults` skips the review (and `--yes` implies it); unknown token pricing is always asked about (or warned about in non-interactive runs).
