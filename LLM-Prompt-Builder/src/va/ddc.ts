  ...
  return {
    ...
    fields: [
      ...
      textField(
        ...
        'Credential domain',
        pb.credentialDomain,
        'SAS Viya credential domain provider API keys are resolved from under the signed-in user; models without a key entry are disabled with a note. Defaults to agentic-ai-keys (the default of the create-credential-domain scripts and mdb credentials-apply); a domain that does not exist resolves to no keys. Enter none to disable credential lookups. See the Managing Credentials administration guide.'
      ),
