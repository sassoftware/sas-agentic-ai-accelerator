...
- **The Builder's new *Enrichment* section maps it.** Each prompt input is filled from a closed vocabulary of things the pipeline already holds — the chunk, the whole document (capped at 20,000 characters), the neighbouring chunks, the heading path, the file name, the source location, the position — then you choose which output becomes the context header and which are kept as tags. Recognisable input names are filled in for you; an unfamiliar one stays **blank and blocks the save**, because a silently wrong mapping produces a whole corpus of confident nonsense
- **Enrichment is part of the configuration fingerprint** — the prompt, the mapping, the selected outputs *and the prompt's own score code*. A context header changes what a chunk **is**, exactly as the chunker and the embedding model do, so turning it on, off, or editing the prompt is refused by the drift guard until the pipeline version is bumped. That forces the re-ingest that keeps a collection consistent, instead of leaving half of it with headers and half without
...
- **`RAG - Enrich Chunks`** is the Studio-flow twin, wired between *Chunk Documents* and *Embed Chunks*. The Builder adds it to the generated flow only for a setup that enriches — a step sitting in a flow doing nothing invites someone to fill it in without the setup knowing
