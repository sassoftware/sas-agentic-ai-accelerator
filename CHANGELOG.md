...
The whole path was **validated end-to-end against a live SAS Viya environment** (dspy 3.2.1): the deployed job definition was launched through the Job Execution REST API with the Builder's exact arguments, optimised a 10-example prompt through the SCR `qwen_25_05b` container with `BootstrapFewShot`, and wrote back the optimised prompt-test (tags + provenance), the optimization-tracker entry (jobId matching the launched job) and the dataset snapshot. Four corrections from that live run are included: the `SCRLM` adapter returns its `usage` as a plain dict (dspy calls `dict(response.usage)`); the Builder resolves the optimize job by **folder membership** because a live `/folders/folders/@item` returns 404 for jobDefinition members; the Builder parses the job log's real shape — a `vnd.sas.compute.log.line` JSON collection — and filters the `%put` source-echo lines so each milestone shows once; and the job stamps tags + provenance (`sourcePromptId`, `optimizedBy`) via the model's **`properties` array** with an ETag'd PUT, since Model Manager silently drops tags and custom fields on the create POST. (Also fixed: multi-line `*` comments in the job program containing semicolons, which put the SAS session into syntax-check mode.)
