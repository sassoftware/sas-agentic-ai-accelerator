...
- **`mdb builders-import` and `mdb package-export`.** Importing the Builder packages is one command: it loads the release table, uploads each package and rewrites the transfer service's import mapping so the report lands bound to `SAS_CAS_LIBRARY.SAS_RELEASES_TABLE` and its Data-Driven Content URL points at `SAS_VIYA_URL`, waits for the job, and with `--options` restores the site's saved builder options in the same run (`--dry-run` shows the mapping). `mdb package-export --folder <SAS Content folder>` is the maintainers' side: export with dependencies and rules, download, rewrite the exporting host to the placeholder inside the compressed report content, check, write to `SAS-Viya-Integrations/` - the three manual steps in one.
