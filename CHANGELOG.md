...
- **The release table knows an upgrade section.** `mdb load-releases` (and the import that runs it) typed every non-standard changelog section as `Changed`; a section named *Upgrading from ...* is now typed `Upgrade`, so the steps a site follows are told apart from the changes it receives in the `ACCELERATOR_RELEASES` table.
