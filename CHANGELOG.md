...
- **The RAG Builder package binds the release table.** Re-exported with `mdb package-export`: its report's only data source is `Public.ACCELERATOR_RELEASES`, which `mdb builders-import` retargets at the site's table; the demo table it used to carry is gone.
